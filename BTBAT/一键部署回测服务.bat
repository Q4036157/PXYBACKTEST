...
set "PXYBACKTEST_VNPY_PYTHON=%BACKTEST_PYTHON%"
set "PXYBACKTEST_DAA_PYTHON=%DAA_PYTHON%"
...
powershell.exe -NoLogo -NoProfile -ExecutionPolicy Bypass -Command "$health=Invoke-RestMethod -Uri 'http://127.0.0.1:%SERVICE_PORT%/health' -TimeoutSec 8;if(-not $health.ok -or -not $health.serviceTokenConfigured -or -not $health.pxydataSnapshotConfigured){Write-Host '[FAIL] Health prerequisites are incomplete.' -ForegroundColor Red;exit 1};$token=(Get-Content -LiteralPath '%SERVICE_TOKEN_FILE%' -Raw).Trim();$headers=@{'X-PXY-Service-Token'=$token;'X-PXY-User-Id'='deployment-check';'X-PXY-Source-Node'='app-win-01'};$cap=Invoke-RestMethod -Uri 'http://127.0.0.1:%SERVICE_PORT%/api/v2/capabilities' -Headers $headers -TimeoutSec 60;$vnpy=$cap.engines|Where-Object id -eq 'vnpy_cta'|Select-Object -First 1;$ashare=$cap.engines|Where-Object id -eq 'a_share_portfolio'|Select-Object -First 1;if($cap.task_contract -ne 'pxybacktest.task-result.v2' -or -not $vnpy.available -or -not $ashare.available -or @($ashare.strategies).Count -lt 1){Write-Host '[FAIL] Capabilities contract/DAA strategy check failed.' -ForegroundColor Red;exit 1};Write-Host ('[OK] Health, PXYDATA, DAA (' + @($ashare.strategies).Count + ' strategies) and capabilities checks passed.') -ForegroundColor Green"
if errorlevel 1 goto :failed
